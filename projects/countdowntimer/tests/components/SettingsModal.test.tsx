...
// Settings modal is fully opaque on mobile viewports
// @criterion-hash: c4loop4mob01
describe('[AC-settings-mobile-1] mobile dialog opacity', () => {
  it('dialog has mobile-specific CSS class for opaque rendering', () => {
    const { container } = render(<SettingsModal {...defaultProps} />);
    const dialog = container.querySelector('dialog');
    // The dialog class must reference the mobile-ready dialog styles
    expect(dialog?.className).toMatch(/dialog/);
  });
});

// @criterion: AC-settings-spacing-1
// Settings sections have adequate spacing (24-32px) between groups
// @criterion-hash: c4loop4spc01
describe('[AC-settings-spacing-1] section spacing', () => {
  it('renders four distinct settings sections', () => {
    render(<SettingsModal {...defaultProps} />);
    expect(screen.getByTestId('settings-section-timing')).toBeInTheDocument();
    expect(screen.getByTestId('settings-section-behavior')).toBeInTheDocument();
    expect(screen.getByTestId('settings-section-sound')).toBeInTheDocument();
    expect(screen.getByTestId('settings-section-notifications')).toBeInTheDocument();
  });
});

// @criterion: AC-settings-anim-1
// Settings modal has close animation
// @criterion-hash: c4loop4anm01
describe('[AC-settings-anim-1] modal close animation', () => {
  it('dialog has closing data attribute when closing', async () => {
    const { container, rerender } = render(<SettingsModal {...defaultProps} isOpen={true} />);
    const dialog = container.querySelector('dialog');
    expect(dialog).toBeInTheDocument();

    // Rerender with isOpen=false to trigger close
    rerender(<SettingsModal {...defaultProps} isOpen={false} />);

    // The dialog should have a data-closing attribute during animation
    expect(dialog?.hasAttribute('data-closing') || !dialog?.hasAttribute('open')).toBe(true);
  });
});
