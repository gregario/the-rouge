...
**A phase times out.**
State is saved to disk after every phase. Nothing is lost. Run `rouge build <project>` to resume from where it left off.

**Evaluation keeps failing on the same gap.**
After 3 retries on the same gap, Rouge transitions to `waiting-for-human` and pings you on Slack. Check the message for what it tried and what's failing. Give feedback via `/rouge feedback <project> "your guidance"` and it resumes. See also [common situations](your-first-product.md#common-situations).
...
Rouge detected nothing to do for this feature area. It advances to the next area automatically. This is normal -- not every cycle produces code.
...
Give feedback via `/rouge feedback <project> "your feedback"`. Rouge incorporates it on the next cycle. For big direction changes, stop the loop, edit `vision.json` directly in the project directory, and restart with `rouge build`.

**I want to skip a feature area.**
Edit the project's `state.json` and set the feature area status to `complete`. Rouge will skip it on the next cycle.
