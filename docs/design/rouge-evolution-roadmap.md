...
- main: **P1.18** (branch `feat/p1.18-gold-set-calibration`) — gold-set + Cohen's Kappa calibration infrastructure. Pure module computes quadratic-weighted Kappa per rubric dimension; CLI `rouge eval-calibrate` gates prompt changes to the judgment layer. `schemas/gold-set-entry-v1.json` + `library/gold-sets/product-eval/README.md` define the entry shape; both blocklisted from self-improve so the pipeline can't retune its own calibration. 40 new tests (33 module + 7 CLI integration). `rouge.config.json → eval_calibration {min_kappa:0.75, min_entries:20}`. Ships empty on purpose — calibration requires real human labels; seeding sample data would game the gate. Side fix: P1.14's `library/rubrics/` was not in the npm `files` array, so the rubric didn't ship with installs; added here along with `library/gold-sets/`.
...
- **P1.18 follow-up** — label the first ~20 gold-set entries from real shipped cycles so the gate can start enforcing. Until then `rouge eval-calibrate` returns exit 2 (insufficient data) as designed.
