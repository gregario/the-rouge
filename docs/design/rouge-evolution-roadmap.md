...
- main: **B1–B4 + D2** (adversarial-review fixes, branch `fix/adversarial-review-2026-04-23`) — language_review ownership consolidated, finding-validator enforces evidence_span invariants (21 tests), divide-by-zero guard on criteria_pass_rate, CI wired via .github/workflows/test.yml, VISION.md principle #3 disambiguated, P1.21 capability-check spec committed.
- main: **P1.21 full** (6 commits, branch `feat/p1.21-capability-check`) — capability-check gate landed end-to-end. Module (5 pure deterministic signals + orchestrator, 38 tests). Runner (collect + context builder, 23 tests). Analyzer Step 0 reads capability_assessments; routes capability-gap findings to notify-human before milestone-fix attempt. Launcher pre-phase hook injects assessments. Schema formalized. Flag-gated via rouge.config.json → capability_check.enabled (default true). 7 flag tests enforce config-vs-module-default sync. Prevents the endless-loop failure mode the owner flagged as costing hundreds of dollars historically.
- main: **P1.16b** (branch `feat/p1.16b-structured-refs`) — structured evidence references replace free-form evidence_span. Judges emit `evidence_ref: {type, path, quote}` on high-confidence findings; launcher resolves the path (JSONPath into cycle_context OR file:lines), verifies quote. Fabricated refs auto-downgrade confidence. Module + deep-validation integration + 02c/02e prompt updates + 77 new tests. Solves the whole-haystack paraphrase problem the original fuzzy design couldn't.
- main: **P1.14** (branch `feat/p1.14-rubric`) — Rouge-native product-quality rubric. `library/rubrics/product-quality-v1.md` defines 6 dimensions (journey completeness, interaction fidelity, visual coherence, content grounding, edge resilience, vision fit) with 0-3 ordinal anchors. Preserves Gary Tan / expand-contract-hold ethos — anchors are visceral ("close the tab", "looks like AI built this"), not checklisty. 02e PO lens rewritten to score against the rubric. Aggregation: verdict (PRODUCTION_READY requires all ≥ 2 + one 3 + zero 0s); confidence = sum/18 normalized. Rubric is human-authored instrument (GC.1 blocklists `library/rubrics/**` from self-improve). 10 tests assert structural invariants + taste ethos preserved.
- main: **P0.6** — MCP health-check wired into `rouge doctor`. Fresh installs now see which MCPs are configured, which are ready, which are missing env vars. Surfaces as a warning (never a blocker) so the user sees what to set up without doctor failing on optional configuration. 5 tests assert: check appears in output, status is never blocker, detail summarizes counts, warnings don't leak into blockers list.
...
- main: **P1.18** (branch `feat/p1.18-gold-set-calibration`) — gold-set + Cohen's Kappa calibration infrastructure. Pure module computes quadratic-weighted Kappa per rubric dimension; CLI `rouge eval-calibrate` gates prompt changes to the judgment layer. `schemas/gold-set-entry-v1.json` defines entry shape; both blocklisted from self-improve so the pipeline can't retune its own calibration. 40 new tests (33 module + 7 CLI integration). `rouge.config.json → eval_calibration {min_kappa:0.75, min_entries:20}`. Side fix: P1.14's `library/rubrics/` was not in the npm `files` array, so the rubric didn't ship with installs; added here along with `library/gold-sets/`.
- main: **P1.18b** (branch `feat/p1.18b-synthetic-gold-set`) — pivoted to synthetic ground truth after owner feedback: "I will never label or answer anything. You can work around that." The original P1.18 design depended on owner hand-labeling 20+ real cycles to bootstrap the gate — a pattern the owner explicitly forbids (see `feedback_no_owner_input_required.md`). Replacement: `src/launcher/gold-set-synth.js` deterministically generates 20 synthetic fixtures whose `cycle_context_excerpt` fields carry rule-derived signals per rubric anchor. Verdict distribution spans PR/NI/NR; every dimension takes ≥ 3 distinct scores across the set (no column-collapse). `rouge eval-seed-gold` regenerates (idempotent, refuses to clobber non-synthetic labelers by default). 22 new tests. Gate is now immediately operable without owner labor — `rouge eval-calibrate --model-labels <file>` works today, exit 2 only when model-labels absent (clear next step, not blocker).
- main: **P1.5R** — supersedes P1.5. Rouge currently has fixed depth (calculator and planning-windows pass through the same pipeline shape). Fix: introduce a categorical project_size dial (XS/S/M/L/XL) set during seeding's new SIZING sub-phase, read by every downstream phase (seeding disciplines, SPEC depth, budget cap, cycle budget). Design doc at `docs/design/adaptive-depth-dial.md`. Initiative scoped as 6 PRs.
  - main: **P1.5R PR 1 — Design doc** (commits 07a1e4c + dc8f502). Design, open questions, and resolutions (sizing as dedicated sub-phase, grow-never-shrink dial, independent dials from profile, BRAINSTORM signals block, XL architecture deferred to issue #202).
  - main: **P1.5R PR 2 — Classifier + signals block** (branch `feat/p1.5r-pr2-classifier`). `schemas/sizing-v1.json` + `src/launcher/project-sizer.js` (pure classifier, human-override, grow-tier, signals-parser) + `## Classifier Signals` block added to BRAINSTORM prompt template. 34 new tests. Ready for PR 3 (SIZING sub-phase) to consume.
  - main: **P1.5R PR 3 — SIZING sub-phase** (branch `feat/p1.5r-pr3-sizing-subphase`). New discipline prompt `src/prompts/seeding/03b-sizing.md` slots between TASTE and SPEC. `rouge size-project` CLI reads BRAINSTORM signals, classifies, writes `seed_spec/sizing.json`, supports `--override` for human redirect. Swarm orchestrator updated 8→9 disciplines + mandatory sequence constraint (TASTE → SIZING → SPEC). All discipline registries synced (bot.js, dashboard types, bridge-mapper, stepper component + test, chat-panel). 12 new CLI integration tests. 477 dashboard tests still green after updating the stepper fixture. Ready for PR 4 (orchestrator skip-by-tier) + PR 5 (SPEC tier-aware depth) + PR 6 (budget/cycle defaults).
  - main: **P1.5R PR 4 — Skip-by-tier** (branch `feat/p1.5r-pr4-skip-by-tier`). New module `src/launcher/discipline-registry.js` authoritative map of discipline → `applicable_at` tier (XS for brainstorming/taste/sizing/spec; S for infrastructure/design/legal-privacy; M for competition/marketing). Orchestrator prompt now tables tier per discipline + explicit skip rule: emit `[DISCIPLINE_SKIPPED: name — reason]` when `applicable_at > project_size`. Marker handler in bot.js sets status='skipped'; resume block distinguishes skipped from complete. Dashboard `DisciplineStatus` gains 'skipped' variant with a Minus-icon stepper rendering. Regression coverage for a legal-privacy parsing bug (regex now requires `\s+` before separator so in-name hyphens stay with the name). 28 new tests (20 registry + 8 regex/skip recording). Sequence constraint: COMPETITION + MARKETING must now run after SIZING (their skip eligibility depends on project_size).
  - main: **P1.5R PR 5 — SPEC tier-aware depth** (branch `feat/p1.5r-pr5-spec-tier-aware`). `src/prompts/seeding/04-spec.md` now reads `seed_spec/sizing.json` before Beat 1, maps tier to FA count + AC range (XS: 1 FA / 2-4 ACs; S: 2-3 / 3-5; M: 3-5 / 3-5; L: 6-8 / 4-6; XL: 8+ / 5+). L/XL activate iterative per-FA mode: per-FA retrieval cycle, FA spec, then a MANDATORY cross-cut pass hunting data-model conflicts + AC contradictions + missing cross-FA journeys + shared-component drift. Depth per FA is tier-invariant — the 3-8 page floor holds at every tier (XS reduces scope, never rigor). Default-to-M fallback when sizing.json is missing keeps existing projects working. 8 new prompt-content tests lock in the guidance (reading sizing.json, all five tiers, cross-cut mandatory, invariant-depth framing, four conflict classes named).
  - main: **Tier-defaults doubled** (commit 100658b). Owner feedback that the initial v1 cap values were too tight. XS $15→$30, S $30→$60, M $50→$100, L $100→$200, XL $250→$500. Monotonicity assertion auto-verified. Existing stamped artifacts keep their old values (frozen at classification); regeneration via `rouge size-project` picks up the new defaults.
- main: **P1.19 PR 7 — 00-foundation-building Opus 4.7 modernization** (commit 5c13c27). Higher-stakes factory prompt. **Deliberate preservation of emphatic register on three load-bearing safety blocks:** (1) the CRITICAL: ISOLATION RULES section (NEVER/ALWAYS rules tied to the destroyed-mtgordle incident — force-push wiped a shipped product's remote), (2) the HARD BLOCK / silent-degradation list (Capability Avoidance Problem), (3) "Do NOT delete failing tests. That is fraud." (integrity rule). The Opus 4.7 "over-triggers on shouty language" effect is actually desirable here — these rules SHOULD trigger maximum caution. Stylistic emphasis softened elsewhere: "MUST NOT scaffold for static-export" → "skips," "Do NOT assume Supabase" → "each project names its own provider," "Do NOT create a new branch" → "don't create another one," Step 11's three Do-NOT operational items reframed as positive directives, "What You Do NOT Build" → "Foundation Scope Boundary" (heading softened, scope-creep rule inside stays emphatic because it's tied to the 0-delta spin incident). 18 tests explicitly assert each safety block stays emphatic AND each stylistic emphasis got softened — locks in both dimensions of the decision.

- main: **P1.19 PR 6 — 07-ship-promote Opus 4.7 modernization** (commit 9d78d8a). "Before doing ANYTHING" → "Before doing anything else in this phase, run two gates in order." "If ANY gate shows false" → "If any gate shows false." "CRITICAL: If promotion fails, do NOT retry automatically" → "Failed promotions escalate; they do not auto-retry or auto-rollback" + added rationale ("A failed production deploy may have partially applied"). Three "do NOT" hedges softened. Scope Boundary rewritten as six positive-lead bullets (run both pre-checks, one attempt then escalate, CLI not slash commands, record outcome for launcher, promote existing history no rewrite, every required gate passed). 20 tests: 10 behavioral contract (ship_result + ship_error + ship_blocked outputs, Gate 1 short-circuit with four OR conditions, Gate 2 five required gates, optional gates, deploy-target dispatch for all 5 platforms, one-attempt-then-escalate, rollback via vendor handler, semver classification), 10 modernization invariants.

- main: **P1.19 PR 5 — 05-change-spec-generation Opus 4.7 modernization** (commit a42c3ad). Two `do NOT` hedges softened. "SINGLE change spec" / "if ANY gap" / "MUST NOT" all-caps emphases de-capitalised. "ONE loop phase permitted" → "the one loop phase permitted" + append-only invariant stated positively. Scope Boundary rewritten as seven positive-lead bullets (write-specs-not-code, flag-design-direction, write-testable-criteria, operate-within-vision-escalate-conflicts, specific-targets-no-one-liners, try-new-approaches-skip-failed, write-files-not-deploys). 19 tests: 9 behavioral contract (change_specs_pending output, change_spec_briefs input, do_not_repeat handling, task_ledger append-only rule, six Depth Standards sections, OpenSpec CLI usage, priority classification, requires_design_mode signal, story-building state transition), 10 modernization invariants.

- main: **P1.19 PR 4 — 03-qa-fixing Opus 4.7 modernization** (commit 807f8a8). Rewrote the Phase Identity opener to drop the four-part No cascade while preserving the surgical-fix framing. Reframed eleven "do NOT" / "Do NOT" hedges as positive directives (0 remaining — asserted by test). "This is non-negotiable" softened to direct contract language; "ANY fix code" → "fix code"; "MUST reference" → "reference"; "Do NOT guess. Do NOT try random fixes. Form ONE hypothesis" → "Form one hypothesis and verify it." Scope Boundary rewritten as five positive-lead bullets (fix bugs QA flagged; restore specified behaviour; deploy to staging; write results; flag ambiguities). 19 tests: 10 behavioral contract (qa_fix_results shape, escalation_needed flag, retry_counts history, three-strikes rule, TDD contract, atomic-commit rule, regression-test auto-gen, blast-radius step, staging-only guarantee), 9 modernization invariants (the zero-"do NOT" assertion is the regression catcher).

- main: **P1.19 PR 3 — 04-analyzing Opus 4.7 modernization** (commit ed7785d). Higher-stakes than retro/docrelease — analyzer routes every cycle. Scope Boundary reframes four explicit negatives as positive-lead directives. Phase-identity line rewritten from three negative clauses to one positive paragraph. "IMPORTANT: ... ALL threshold checks" → calm bold + sentence-case. Three "do NOT" all-caps hedges softened to "don't" in prose. 18 new tests: 9 behavioral contract (analysis_recommendation shape, capability_assessments integration, five capability signals, capability-gap → notify-human routing, confidence_adjusted threshold rule, all seven recommendation verbs, root_cause fallback for audit-recommender, mid_loop_correction + foundation_scope fields), 9 modernization invariants (including an explicit "do NOT should not appear" check to catch regressions).

- main: **P1.19 PR 2 — 08-document-release Opus 4.7 modernization** (commit c235d9e). Scope Boundary section with positive leads; docs-only guarantee preserved; "log subjective changes, don't auto-apply" rule preserved; "update the delta, not a rewrite" preserved. 15 new tests (9 behavioral contract — doc_release_result, doc_subjective_changes, doc_consistency_fixes, every audited doc file still named; 6 modernization invariants).

- main: **P1.19 PR 1 — 09-cycle-retrospective Opus 4.7 modernization** (commit c957a80). Reframed "What You Do NOT Do" as "Scope Boundary" with positive-alternative leads. "No team language" → "Use system-actor language." "Never promote to active" → "Draft as shadow / proposed; promotion is a human-reviewed PR, never an autonomous step." "No speculative amendments" → "Require ≥3 cycles of consistent signal per amendment." 18 new tests lock in (a) behavioral contract the launcher's post-retrospective hook depends on (every field it reads — structured_retro, amendments_proposed, journey_entry, retro_metrics, trend_snapshot, prompt_improvement_proposals, root_cause taxonomy, shadow-only promotion guarantee, 3-cycle evidence gate), (b) modernization patterns (no "think step by step," no "CRITICAL:", no "YOU MUST," Scope Boundary framing present). Note: XML tag structure NOT added to this prompt — retro reads cycle_context and writes JSON, no external content injection risk where XML boundaries would help. XML adoption reserved for prompts with genuine input-distinctness needs.

  - main: **P1.5R PR 6 — Tier-based budget defaults** (branch `feat/p1.5r-pr6-tier-defaults`). New module `src/launcher/tier-defaults.js` with TIER_DEFAULTS map (XS: $30/2 cycles, S: $60/3, M: $100/5, L: $200/8, XL: $500/12 — doubled from the initial v1 values after owner feedback that the originals were too tight). `project-sizer.js classify()` / `applyHumanOverride()` / `growTier()` now stamp `defaults` onto the sizing artifact (schema extended as optional for back-compat). `rouge-loop.js` budget resolution gains tier fallback: `state.budget_cap_usd ?? sizingDefault ?? config.budget_cap_usd` — per-project human override always wins, then tier, then global. Applies to both hard-cap gate and cost-alert thresholds. 17 new tests (14 tier-defaults + 3 classifier defaults-field + regression that legacy sizing.json without stamped defaults still resolves via tier lookup). **P1.5R initiative complete.**
...
- **P1.18c** — wire `rouge eval-calibrate --run-judge` path that invokes the judge on each fixture's cycle_context_excerpt, captures model labels, runs calibration end-to-end in one command. Currently the model-labels step is out-of-band. This is harness-dependent (invokes `claude -p`) so it partially blocks on P5.9, but a prototype using the current subprocess spawn path is worth building.
...
## Where we are on 2026-04-24 (session handover)

**Session summary:** 17 commits landed on main — P1.18b pivot, full P1.5R initiative (6 PRs + doubled budgets), and P1.19 sweep through 7 of 17 loop prompts.

**Landed this session (commit order):**

| Commit | What |
|---|---|
| `3d2f4fe` | P1.18b — synthetic gold-set (pivoted from P1.18 after "I won't label data" feedback; 20 deterministic fixtures) |
| `07a1e4c` | P1.5R design doc |
| `dc8f502` | P1.5R design doc Q1-Q5 resolutions |
| `020ebf9` | P1.5R PR 2 — classifier module + sizing-v1 schema + BRAINSTORM signals block |
| `a3e82d8` | P1.5R PR 3 — SIZING sub-phase + `rouge size-project` CLI (8→9 disciplines) |
| `81bc782` | P1.5R PR 4 — orchestrator skip-by-tier + [DISCIPLINE_SKIPPED] marker (caught legal-privacy regex bug) |
| `bdb65a7` | P1.5R PR 5 — SPEC tier-aware depth (L/XL iterative per-FA + mandatory cross-cut) |
| `8592868` | P1.5R PR 6 — tier-based budget defaults (initiative complete) |
| `100658b` | tier-defaults doubled (owner feedback — XS $30 through XL $500) |
| `c957a80` | P1.19 PR 1 — 09-cycle-retrospective modernization |
| `c235d9e` | P1.19 PR 2 — 08-document-release modernization |
| `ed7785d` | P1.19 PR 3 — 04-analyzing modernization |
| `807f8a8` | P1.19 PR 4 — 03-qa-fixing modernization |
...
| `0657318` | P1.19 PR 9 — seeding/08-infrastructure.md modernization (small technical prompt; no incident-tied safety blocks; all emphasis softened; 21 tests) |
...
| `e3b5d9b` | P4.1 wave 6 — five tier-2 (Auth.js, SendGrid, Convex, Turso, AWS S3) + five matching tier-3 patterns (authjs-oauth-with-drizzle, sendgrid-dynamic-templates, convex-realtime-query-mutation, turso-embedded-replicas, aws-s3-presigned-uploads). Tier-2 23→28; tier-3 15→20. |
| `2dc63bd` | P4.1 wave 7 — final catalogue wave. Four tier-2 (Liveblocks, Replicate, Slack API, Stripe Connect) + four tier-3 patterns. Tier-2 28→32; tier-3 20→24. |
| _next_ | **P5.9 PoC — harness adapter** for non-tool-using phases. Adds `@anthropic-ai/sdk` dep + `src/launcher/harness/sdk-adapter.js` (runPhaseViaSdk with prompt caching + structured output via tool_choice forcing) + 29 unit tests with mocked client + `rouge harness probe` CLI for human end-to-end validation + design doc at `docs/design/harness-poc.md` covering the migration path. Demonstrates 2 of the 7 features blocked by `claude -p` (caching, structured output); 5 remain (citations, batch, MCP per-spawn, multi-Claude orchestration, cost forecasting) — each is a separate sub-PoC scope. Out of scope: migrating tool-using phases (build/fix/deploy keep `claude -p`). |

**Memory work this session:**

- New memory `feedback_no_owner_input_required.md` — "direction yes, grunt work no." Absorbs the older `feedback_no_pr_review.md` (merged + deleted 2026-04-24). Owner answers direction/taste/what's-next; AI handles labeling / form-filling / line-by-line review or redesigns to not need them.
- Memory audit of all 17 files: 13 kept as-is, 1 merged (above), 1 updated (`project_supabase_new.md` refreshed to 2026-04-24, noted construction-coordinator as first-planned Supabase Auth use), 2 verified and kept (TASTE framing via `seeding/03-taste.md`, Vercel hobby via `deploy-to-staging.js:104`). MEMORY.md index is 16 entries.
...
**P1.19 initiative: COMPLETE.** All 26 prompts modernized across three stages.
...
**Direction calls made with owner this session (for traceability):**
- P1.18 → P1.18b pivot approved (synthetic over human labels, drove the "no owner input required" memory update)
- P1.5R adaptive depth dial approved (supersedes P1.5's narrow iterative-spec framing)
- Q1–Q5 resolutions on P1.5R design doc (sizing as sub-phase; grow-never-shrink; independent profile × size dials; signals block on BRAINSTORM; XL architecture deferred to #202)
- Tier-defaults doubled
- P1.19 pattern: XML tags only where they buy distinctness; judge prompts deferred to dedicated PR set; generation sweep in ascending-file-size-ish order
- Memory audit: fold `feedback_no_pr_review` into `feedback_no_owner_input_required`; verify rows 8, 16, 17; keep 13 as-is

**No unresolved direction questions at handover.**
