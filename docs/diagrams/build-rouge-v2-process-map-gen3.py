...
def build():
    ...
    # Change Spec → Story: go UP from CS top, then LEFT to the "next story" loop
    # path (same x as the loop_left_x), then UP to story left-centre level,
    # then RIGHT into story left edge. This shares the return path with "next story".
    cs_tx, cs_ty = top_("cs")
    s_lx, s_ly = lt_("story")
    # loop_left_x is already computed above (left of story box)
    halfway_y = cs_ty + (s_ly - cs_ty) * 0.5  # halfway between CS and Story
    ...
    # Exit bottom of VC, go down to escalation level, then right to escalation left edge
    sx, sy = bot("vc")
    ex, ey = lt_("esc4")
    ...
    # FR → Change Spec (refine): exit TOP of FR, route left, up to CS LEFT input
    # Red dashed (it's a failure/retry path)
    fr_tx, fr_ty = top_("fr")
    cs_lx, cs_ly = lt_("cs")
...
if __name__ == "__main__":
    scene = build()
    out = Path(__file__).parent / "rouge-v2-process-map-gen3.excalidraw"
